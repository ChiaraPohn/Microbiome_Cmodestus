...
#save file so that DADA2 doesn't need to run again
...
#experiments other than LS as controls: 
sd <- sample_data(ps)
sd$Controls_LS <- ifelse(
  sd$Control == "yes", "yes",
  ifelse(sd$Experiment == "LS", "no", "yes")
)
...
ggplot(data=decontam, aes(x=Index, y=LibrarySize, color=Controls_LS)) + 
  geom_point()+
...
contamdf.prev0.05 <- isContaminant(ps, method="prevalence", neg="is.neg", threshold = 0.05)
contamdf.prev0.2 <- isContaminant(ps, method="prevalence", neg="is.neg", threshold = 0.2)
contamdf.prev0.5 <- isContaminant(ps, method="prevalence", neg="is.neg", threshold = 0.5)
contamdf.prev0.6 <- isContaminant(ps, method="prevalence", neg="is.neg", threshold = 0.6)



# Number of contaminating contigs:
table(contamdf.prev$contaminant)
table(contamdf.prev0.05$contaminant)
table(contamdf.prev0.2$contaminant)
table(contamdf.prev0.5$contaminant)
table(contamdf.prev0.6$contaminant)
...
ps.pa.neg <- prune_samples(sample_data(ps.pa)$Controls_LS == "yes", ps.pa)
ps.pa.pos <- prune_samples(sample_data(ps.pa)$Controls_LS == "no", ps.pa)
...
ps.noncontam
ps.noncontam <- prune_samples(sample_data(ps)$Controls_LS!='yes', ps.noncontam)
ps <- ps.noncontam
ps
...
#only relevant for this study:
sum.check <- taxa_sums(ps_LS)
